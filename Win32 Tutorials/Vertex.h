...
namespace rsexp
{
	...
	public:
		...
		public:
			...
			size_t GetOffsetAfter() const noexcept (!IS_DEBUG) { return offset + Size(); }
			size_t GetOffset() const { return offset; }
			size_t Size() const noexcept(!IS_DEBUG) { return SizeOf(type); }

			static const size_t SizeOf(ElementType type) noexcept(!IS_DEBUG) {
	...
	public:
		...
		const Element& ResolveByIndex(size_t i) const noexcept(!IS_DEBUG) { return elements[i]; }
	...
	//acts as a viewpoint into a specific vertex in the buffer
	class Vertex {
		friend class VertexBuffer;

	public:

		//Allows us to get a singular vertex attribute so vertex[2].Attr<Position3D>() = etc...
		template<VertexLayout::ElementType Type>
		auto& Attr() noexcept(!IS_DEBUG)
		{
			using namespace DirectX;
			const auto& element = layout.Resolve<Type>();
			//pData is a pointer to the start and then add the offset to get to the correct attribute
		...
		//in templating && allows us to forward a parameter without modification or unnecessary copies
		template<typename T>
	...
	class VertexBuffer {
	public:
		VertexBuffer(VertexLayout layout) noexcept(!IS_DEBUG) : layout(layout) {}


		const char* GetData() const noexcept(!IS_DEBUG) { return buffer.data(); }

		const VertexLayout& GetLayout() const noexcept { return layout; }
		size_t Size() const noexcept(!IS_DEBUG) { return buffer.size() / layout.Size(); } //size in vertices
		size_t SizeBytes() const noexcept(!IS_DEBUG) { return buffer.size(); } //size in bytes
		...
		//Gets the back of the vector
		Vertex Back() noexcept (!IS_DEBUG) {
			assert(buffer.size() != 0u);
			return Vertex{ buffer.data() + buffer.size() - layout.Size(), layout };
		}

		//Gets the front of the vector
	...
	private:
		std::vector<char> buffer; //a vector of bytes of all vertices
		VertexLayout layout;
	};
}
