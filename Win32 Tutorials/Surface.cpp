...
Surface::Surface(unsigned int width, unsigned int height)
{
	...
	if (FAILED(hr)) {
		throw Surface::Exception(__LINE__, __FILE__, "Failed to initialize Scratch Image", hr);
	}
}
...
Surface::Exception::Exception(int line, const char* file, std::string note, std::optional<HRESULT> hr /*= {}*/) noexcept
	: RedSkyException(line, file), note("[Note]" + note)
{
	if (hr) {
...
const char* Surface::Exception::what() const noexcept
{
	...
	oss << RedSkyException::what() << std::endl << GetNote();
	whatBuffer = oss.str();
